...
误情况除外）。
6.
在文档中提到的“商户”、“会员”代指业务使用者。“服务商”、“供货商”、“服
务提供商”代指业务接口服务提供商。
7.
接口中状态值或者充值状态（如：Code,OrderState）如果实际返数据异常或非文档描述
...
func (c *Client) Request(path string, params string, result interface{}) (err error) {
	...
	plog := log.WithField("channel", "onfishes").WithField("url", url)
	plog.WithField("requestBody", params).Info("请求报文")
	...
	plog.WithField("responseBody", string(rbytes)).Info("响应报文")
...
// 私钥解密
func RsaDecrypt(val, priKey string) (plainText string) {
	var err error
	defer func() {
		if err != nil {
			log.Error(err)
